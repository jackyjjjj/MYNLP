...
save_path = r"D:\pythoncode\pythonProject\data"
pattern = re.compile(
    r"(?P<userName>.*?)\n"  # 昵称
    r"(?P<rating>\d+星)\n"  # 评分
    r"(?P<content>.*?)\n"  # 评论内容
...
if __name__ == "__main__":
    ...
    #点击分类
    driver.find_element(By.XPATH,
                        '(//android.widget.ImageView[@resource-id="com.bbk.appstore:id/lottie_view"])[4]').click()
    time.sleep(2)
    for c in categories:
        driver.find_element(By.XPATH,
                            f"//android.widget.TextView[@resource-id='com.bbk.appstore:id/tvCategoryFirstColumngText' and @text='{c}']").click()
        for i in range(1, 4):  # 遍历 1,2,3
            time.sleep(0.5)
            driver.find_element(By.XPATH,
                                f"(//android.widget.LinearLayout[@resource-id='com.bbk.appstore:id/ll_info'])[ {i} ]").click()
            time.sleep(0.5)
            driver.tap([(450,510)])
            getComments(driver)
            time.sleep(1)
